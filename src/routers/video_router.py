...
async def get_video(video_id: int,db: Session = Depends(get_db), current_user: User = Depends(verify_token)):
    ...
    votes_data = get_votes_by_video_id(video_id)
    video_response = VideoResponse.model_validate(exist_video)
    video_response.votes =votes_data["votes_count"]

    return video_response



def get_votes_by_video_id(video_id: int, db: Session = Depends(get_db)):
    exist_video = db.query(Video).filter(Video.id == video_id).first()
    if not exist_video:
        raise HTTPException(status_code=404, detail="Video not found")
